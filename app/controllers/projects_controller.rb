class ProjectsController < ApplicationController
  ...
  before_action :set_tenant, only: [:show, :edit, :destroy, :new, :create, :users, :add_user]
  before_action :verify_tenant

  # GET /projects
  # GET /projects.json
  def index
    @projects = Project.by_user_plan_and_tenent(params[:tenant_id], current_user)
  end
  ...
  def add_user
    @project_user = UserProject.new(user_id: pararms[:user_id], project_id: @project.id)

    responde_to do |format|
      if @project_user.save
        format.html {
          redirect_to users_tenant_project_url(id: @project.id, tenant_id: @tenant_id), notice: "User was successfully added to project"
        }
      else
        format.html { 
